...
static int ov5648_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		ret = ov5648_write_array(ov5648->client, sensor_oe_enable_regs);
		if (ret) {
...
static int ov5648_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_EXPOSURE_AUTO:
		ret = ov5648_read_reg(ov5648->client, OV5647_REG_AEC_AGC, OV5648_REG_VALUE_08BIT,&value);
		printk("exposure auto regval=%x, ready to set:%x\n",value, ctrl->val == V4L2_EXPOSURE_MANUAL ? value | BIT(0) : value & ~BIT(0));	
		
		ret = ov5648_write_reg(ov5648->client, OV5647_REG_AEC_AGC,
						 OV5648_REG_VALUE_08BIT, ctrl->val == V4L2_EXPOSURE_MANUAL ? value | BIT(0) : value & ~BIT(0));
		break;
#endif
	case V4L2_CID_EXPOSURE:
		/* 4 least significant bits of expsoure are fractional part */
		printk("TTT set exposure:val=%x\n",ctrl->val);
...
static int ov5648_initialize_controls(struct ov5648 *ov5648)
{
	...
	/* auto exposure*/
    v4l2_ctrl_new_std_menu(handler, &ov5648_ctrl_ops,
